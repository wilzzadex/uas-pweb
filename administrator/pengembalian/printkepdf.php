...
$mahasiswa = mysqli_query($conn, "select * from orders where status='BARANG SUDAH DI KEMBALIKAN'");
while ($row = mysqli_fetch_array($mahasiswa)){
	...
	if($row['denda']=='0'){
		$pdf->Cell(60,6,"Tidak Ada Denda",1,0);
	}else{
		$pdf->Cell(60,6,"Rp.".number_format($row['denda'],2,',','.'),1,0);
	}
